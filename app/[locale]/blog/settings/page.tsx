...
export default function BlogSettingsPage() {
  ...
  const rowClass =
    "focus-ring flex items-center justify-between gap-3 rounded-xl px-3 py-3 text-left text-sm text-slate-700 transition-colors hover:bg-slate-50";

  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      <h1 className="text-2xl font-bold tracking-tight text-slate-900">{t("settingsTitle")}</h1>
      <p className="mt-1 text-[13px] text-slate-500">{t("settingsSubtitle")}</p>

      {/* 계정 */}
      <section className="mt-8">
        <h2 className="mb-3 text-sm font-semibold text-slate-700">{t("settingsAccount")}</h2>
      ...
      {/* 화면 */}
      <section className="mt-8">
        <h2 className="mb-3 text-sm font-semibold text-slate-700">{t("settingsDisplay")}</h2>
        <div className="rounded-2xl border border-slate-200 p-2">
          <ThemeToggle className={rowClass} />
          <div className="px-3 pb-1 pt-3 text-[11px] font-medium text-slate-400">
            {tLang("label")}
          </div>
          {routing.locales.map((l) => (
            <button
              ...
              className={cn(rowClass, "w-full", l === locale && "text-accent-700")}
            >
              {tLang(l)}
      ...
      <section className="mt-8">
        <button
          ...
          className="focus-ring inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-600"
        >
