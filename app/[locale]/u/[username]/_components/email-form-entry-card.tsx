          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="mt-2 flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className={`h-10 flex-1 rounded-lg border bg-white/90 px-3 text-sm text-slate-900 outline-none ring-accent-600 placeholder:text-slate-400 focus:ring-2 ${colors.cardBorder}`}
              />
              <button
                type="submit"
                disabled={status === "submitting"}
                className={`h-10 rounded-lg px-4 text-[13px] font-medium transition active:scale-[0.98] disabled:opacity-60 ${colors.ctaPrimary}`}
              >
