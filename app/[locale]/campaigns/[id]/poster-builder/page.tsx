...
export default function PosterBuilderPage() {
  ...
  const handleFile = useCallback(
    async (file: File | null) => {
      ...
      try {
        ...
        toast("PDF 를 읽지 못했어요 — 손상된 파일일 수 있어요", "error");
      }
    },
    [toast],
  );
  ...
  async function compose() {
    ...
    try {
      ...
      toast(`${batches.length}개 페이지 합본 PDF 를 만들었어요`, "success");
    } catch (e) {
      toast(e instanceof Error ? e.message : "합성 실패", "error");
    } finally {
      setComposing(false);
    }
  }
  ...
  return (
    <div className="container max-w-5xl space-y-6 py-10">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          {loading ? (
            ...
            <>
              <span className="font-medium text-slate-700">{campaign.name}</span> · 캠페인의 모든
              배포 묶음 ({batchCount}개) 마다 PDF 페이지 복제 + 각자 다른 QR 박힘. 합본 PDF 로
              다운로드.
            </>
          ) : null}
        </p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/60 px-4 py-3 text-[12px] leading-snug text-slate-600">
        Canva·인디자인·PowerPoint 같은 도구에서 디자인을 끝내고 PDF 로 내보낸 뒤 여기서 QR
        위치만 정하세요. 우리는 디자인 도구가 아니라 좌표 합성기.
      </div>
        ...
        <aside className="space-y-3">
          <div className="rounded-2xl border border-slate-200 bg-white px-4 py-4">
            ...
            <p className="mt-2 text-sm text-slate-700">
              <span className="font-medium text-slate-900">{batchCount}개</span> 배포 묶음 ·
              결과는 <span className="font-medium text-slate-900">{batchCount}페이지</span> PDF
            </p>
            {batchCount === 0 && (
              <p className="mt-2 text-[11px] text-rose-600">
                먼저 배포 묶음을 추가하세요. (QR 캠페인 상세 → 배포 묶음 추가)
              </p>
            )}
          </div>
          ...
          >
            ...
            {composing ? "합성 중..." : `합본 PDF 다운로드 (${batchCount}페이지)`}
          </Button>

          <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-[11px] leading-snug text-slate-500">
            QR 위치는 저장하지 않습니다. 같은 PDF 로 다시 들어와도 박스를 다시 그려야 해요. 캠페인은
            일회성 발주가 기본이라 영구 저장은 over.
          </div>
...
function DropZone({ onPick }: { onPick: (file: File) => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm font-medium text-slate-900">PDF 끌어다 놓거나 클릭해서 업로드</p>
      <p className="mt-1 text-[12px] text-slate-500">1페이지 PDF · Canva/인디자인 export 권장</p>
    </label>
  );
}
