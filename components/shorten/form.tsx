...
export function ShortenForm({ authenticated, onShortened }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const code = authenticated && customCode.trim() ? customCode.trim() : undefined;
      const expiry = authenticated && expiresAt ? new Date(expiresAt).toISOString() : undefined;
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      {authenticated && (
        <div>
           ...
           * Alternatives considered (`max-height: 9999px`, JS-measured max-height, display toggle)
           * all had jank or fragility — grid-rows is one declarative CSS rule and survives every
           * inner-content variant without JS bookkeeping. Reduce-motion users skip the animation
           * via the media query collapse on the same className.
           */}
          <div
