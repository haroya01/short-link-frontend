 ...
 *
 * <p>SDK init runs once at module load (guarded by typeof window so SSR doesn't trip on it).
 * DSN comes from {@code NEXT_PUBLIC_POSTHOG_KEY} env var — when absent the SDK is silently
 * skipped so PR previews / local dev without analytics don't error and don't pollute the
 * production project with test traffic.
 *
 * <p>Pageview tracking is manual (not the SDK default) because Next.js App Router doesn't
 * fire a fresh page load on client navigation, so the auto-pageview SDK would only catch the
 * first load. We re-fire on every {@code usePathname} change.
 */
