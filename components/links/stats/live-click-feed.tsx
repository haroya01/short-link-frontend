...
export function LiveClickFeed({ shortCode, onTick }: { shortCode: string; onTick?: () => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        <p className="rounded-xl border border-dashed border-slate-200 dark:border-slate-800 px-4 py-10 text-center text-[12px] text-slate-500 dark:text-slate-400">
          {t("waiting")}
        </p>
      ) : (
