...
작업 중 눈에 띈 동작·UX 이슈를 누적한다. 코드로 고치지 않는다.

1. **북마크 상태가 두 메커니즘으로 갈라져 있음.** 피드 카드의 북마크(`FeedCardBookmark` → `useBookmarks` 공유 스토어)와 글 상세 페이지의 `BookmarkButton`(자체 `getBookmarkStatus(postId)` 로컬 상태)이 서로 다른 출처를 본다. 글 상세에서 북마크를 토글해도 같은 세션의 피드 카드 스토어에는 즉시 반영되지 않고(반대도) 새로고침 전까지 어긋날 수 있다. PR A에서는 둘 다 동작/마크업을 보존만 했고 통합하지 않음 — 통합하려면 상세 버튼도 공유 스토어를 쓰게 해야 하나, 키(`username/slug`)를 상세 페이지에서 확보해야 해 별도 검토 필요.
2. **`like` 버튼 낙관적 카운트가 서버 응답으로 덮어써짐.** 토글 시 `+1` 낙관 표시 후 `likePost` 응답의 `likeCount`로 재설정한다. 서버 지연/동시성 상황에서 숫자가 한 번 튈 수 있음(예: 다른 사용자의 좋아요가 반영되며 +1→+2). 의도된 reconcile 동작이라 보존했고, 보고 차원에서만 기록.
