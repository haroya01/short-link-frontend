...
// A connection's flat block shape — only the fields for `blockType` matter. The mock also stashes the
// block's `refId` (not part of the wire {@link Connection}) so `mockMineCollections(block)` can resolve
// which collections already hold a given block and hand back its `connectionId` — the "이미 담김" signal.
type MockConnection = Connection & { refId?: number };
function hl(id: number, refId: number, quote: string, postTitle: string, username: string, slug: string, why: string): MockConnection {
  return { id, refId, blockType: "HIGHLIGHT", why, title: postTitle, excerpt: null, slug, username, quote, body: null };
}
...
// the bridge from the previous sentence to this one. (The refId args are the mock highlight ids the
// steps stand on — just decorative here; membership is only exercised via POST id 3 below.)
const pathConnections: MockConnection[] = [
  hl(101, 501, Q_PROCESS, "헥사고날 아키텍처, 작은 서비스에 과했을까", "haruka", "hexagonal-too-much",
    "결정을 글로 남기는 습관이 이 길의 출발점이었다."),
  hl(102, 502, Q_MEASURE, "Spring Boot 트랜잭션 전파, 다시 정리", "dohyun", "spring-tx-propagation",
    "그래서 바꾸기 전에 먼저 같은 조건으로 재 봤다."),
  hl(103, 503, Q_SIMPLE, "헥사고날 아키텍처, 작은 서비스에 과했을까", "haruka", "hexagonal-too-much",
    "측정이 내린 결론은 늘 같았다 — 단순함."),
  note(104, "여기까지가 '과정을 남긴다 → 측정한다 → 단순함이 이긴다'로 이어지는 한 흐름. 다음엔 이걸 팀에 어떻게 옮길지.", null),
];

const PATH: MockCollectionDetail = {
  id: 1,
  title: "결정을 남기는 법",
...
const collectionConnections: MockConnection[] = [
  // POST refId 3 = the mock post "hexagonal-too-much" (SEEDS index+1). Seeded already-connected so the
  // connect sheet, opened from that post, shows this collection as "이미 담김" + an unlink affordance.
  post(201, 3, "헥사고날 아키텍처, 작은 서비스에 과했을까", "포트·어댑터로 갈라놓고 6개월 뒤 다시 본 솔직한 회고.",
    "haruka", "hexagonal-too-much", "작은 서비스에 무엇이 과했는지 다시 읽으려고 담아둔 글."),
  hl(202, 599, "좋은 이름은 주석을 지운다.", "리팩터링: 이름 짓기에 하루를 쓰는 이유", "dohyun", "naming-things",
    "북마크가 아니라, 왜 담았는지가 남는 게 컬렉션."),
];

const COLLECTION: MockCollectionDetail = {
  id: 2,
  title: "프로덕트 노트",
...
const myHighlights: MyHighlightItem[] = [
  ...
  },
  {
    ...
    note: null,
    postUsername: "dohyun",
    postSlug: "spring-tx-propagation",
...
type MockCollectionDetail = Omit<CollectionDetail, "connections"> & { connections: MockConnection[] };

function toSummary(
  c: MockCollectionDetail,
  block?: { blockType: Connection["blockType"]; refId: number },
): CollectionSummary {
  // When asked about a specific block, resolve whether this collection already holds it and hand back
  // that connection's id (the "이미 담김" signal); otherwise leave `connectionId` off entirely.
  const held = block
    ? c.connections.find((x) => x.blockType === block.blockType && x.refId === block.refId)
    : undefined;
  return {
    id: c.id,
    title: c.title,
    ...
    preview: c.connections
      ...
      .filter(Boolean),
    ...(block ? { connectionId: held ? held.id : null } : {}),
  };
}

const mockStore: MockCollectionDetail[] = [PATH, COLLECTION];
let mockSeq = 9000;

export function mockMineCollections(block?: {
  blockType: Connection["blockType"];
  refId: number;
}): CollectionSummary[] {
  return mockStore.map((c) => toSummary(c, block));
}
...
export function mockCreateCollection(payload: NewCollection): CollectionSummary {
  const created: MockCollectionDetail = {
    id: ++mockSeq,
    title: payload.title,
...
export function mockConnect(
  ...
  if (!target) return;
  // Idempotent, like the real endpoint: a re-connect of the same block just refreshes its `why`.
  const existing = target.connections.find(
    (x) => x.blockType === payload.blockType && x.refId === payload.refId,
  );
  if (existing) {
    existing.why = payload.why?.trim() || null;
    return;
  }
  // Best-effort label so the new connection shows in the detail/preview (demo only). Stamp `refId` so a
  // follow-up mine-with-block resolves this collection as "이미 담김".
...
export function mockReorderConnections(collectionId: number, connectionIds: number[]): void {
  ...
  target.connections = connectionIds.map((id) => byId.get(id)).filter((x): x is MockConnection => !!x);
}
