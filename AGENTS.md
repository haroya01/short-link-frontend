...
- 무대 연출 신규 추가는 **`useStageVariant()` "on" 뒤에서만**. off(기본)면 도입 전과 픽셀 동일 = 머지 무위험 + 즉시 롤백.
...
- 재질은 **`.glass-chrome`(바 계열) / `.glass-capsule`(알약 계열)** 두 클래스만 소유한다(globals.css) — `bg-white/85 + backdrop-blur` 류 유사-유리를 컴포넌트에 직접 바르지 말 것.
...
- **상단 크롬 축소(condensed) 패턴**: "정적 콘텐츠 + 플레이트 2장 크로스페이드". 콘텐츠 행은 h-14 고정으로 한 픽셀도 안 움직이고, 풀폭 바 판과 떠 있는 캡슐 판(inset-x-3·max-w-[1248px]·rounded-full·상하 1.5 인셋)이 `opacity/transform/visibility` 로만 교차 — **컴포지터 전용이라 120Hz 에서도 매끈**. 레이아웃 속성(margin/width/height/radius) 전환 ❌ — 매 프레임 리플로우+유리 재연산으로 저프레임화된 전력. 상태원은 `useCondensedChrome`(히스테리시스 32/8px). nav.tsx·app-header.tsx 가 기준 구현.
