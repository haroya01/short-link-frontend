...
### 10.7 모션 규칙
- **이징은 한 곡선**: `var(--ease)` (`cubic-bezier(0.16, 1, 0.3, 1)`, globals.css `:root`). Tailwind arbitrary 에선 `ease-[var(--ease)]` / `animate-[name_240ms_var(--ease)_both]`, 인라인 스타일에선 `"transform 320ms var(--ease)"`. framer-motion 처럼 var() 를 못 받는 곳만 `[0.16, 1, 0.3, 1]` 배열로 미러링하고 주석으로 출처를 남긴다. 새 cubic-bezier 하드코딩 ❌ (예외: §0 잠긴 warp 키프레임, 테마 wipe 의 의도된 soft 곡선).
...
### 10.9 활자 두 목소리 (serif = 콘텐츠, sans = 제품)
- **`font-serif`(마루 부리, `--font-serif`) = 콘텐츠가 말할 때만**: 글/시리즈 **제목**(카드·상세·피드 행·다음 화), 본문 헤딩(h2–h4), 피드 마스트헤드, 작가 헤더, 태그 페이지 제목, 에디터 제목 input(발행본 동일성).
- **Pretendard(sans) = 제품이 말할 때**: 버튼, 메타(작가·날짜·카운트), 내비/탭, 칩, 워크스페이스/설정 UI, 본문 문단. excerpt 도 sans — 제목만 세리프인 대비가 잡지 페어링의 핵심.
- serif 표면 규칙: 제목 웨이트는 **600/700**, 400 은 **인용(blockquote) 전용**(이 셋만 로드, `globals.css` `@font-face`), 자간은 `tracking-display`(-0.01em — `tracking-headline`/`tracking-tight` 는 Pretendard 기준이라 세리프엔 과함), 14px 이하 밀집 리스트(레일·에피소드 목록 등)는 sans 유지.
- 인쇄물 어휘: 인용 = 세리프 풀쿼트(한글 italic 합성 기울임 금지), 구분선(hr) = 애스터리즘 `✳ ✳ ✳`, 본문 링크 밑줄 = `decoration-1` + `underline-offset-[0.2em]`. 에디터(`.tiptap`)는 발행본과 동일하게.
- 서양 세리프(이탤릭 디스플레이) 재도입 금지 — 한글 우선 화면에서 겉돌아 폐기된 전력(tailwind.config 주석).
