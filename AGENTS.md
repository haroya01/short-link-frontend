...
- **상단 크롬 축소(condensed) 패턴**: 헤더는 "투명 sticky 외곽 + 유리 내부 셸" 구조. 스크롤 시작 시(`useCondensedChrome`, 히스테리시스 32/8px) 셸이 `mt-2 max-w-5xl rounded-full border + 부양 그림자` 캡슐로 축소, 높이 14→12. 전환은 geometry 속성만(`margin/max-width/border-radius/box-shadow/height`) — **backdrop-filter 는 전환 대상에 넣지 말 것**. nav.tsx·app-header.tsx 가 기준 구현.
