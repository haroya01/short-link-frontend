...
> **블로그 홈의 글 목록도 예외 없이 같은 컬럼이다.** 최신/인기/추천/팔로잉 탭과 검색 결과 모두 `FeedList`/`FeedCard` 목록 행, 탭 헤더도 `max-w-2xl`. 예전 메이슨리 카드 그리드는 코드 스크린샷 커버가 제목보다 먼저 보이고 흰 카드와 어두운 카드가 섞여 어수선해서 철거했다(2026-09). 시리즈 탭도 같은 `max-w-2xl` 안에서 시리즈 덱 카드를 2열로 둔다 — 탭을 바꿔도 머리글이 움직이지 않는다.
>
> **최신·인기 피드는 시리즈를 한 줄로 접는다.** 같은 시리즈의 글은 가장 최근 편 하나만 오고(백엔드), 그 행 메타 아래에 `시리즈 · {제목} · {n}편` 줄이 시리즈 페이지로 이어진다. 태그·검색·작가 면은 편마다 행이 따로 나온다.
>
...
- 2026-09: 블로그 홈 글 목록 전 뷰포트 `FeedCard` 행(메이슨리 철거), 최신·인기 피드 시리즈 접기 + 시리즈 줄(§10.1)
