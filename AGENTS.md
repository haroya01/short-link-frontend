...
- 모든 레일/섹션 라벨(작가·주제·시리즈·태그·아카이브·최근 글)은 **`<RailHeading>`** 사용 — 앞에 **브랜드 그린 마커**(`h-3 w-[3px] bg-accent-500`) + `text-[13px] font-bold text-slate-700`.
...
- 본문 17px / `leading-[1.85]` / `slate-700`(가독 우선), 헤딩 h2→h4 contiguous(`post-blocks.tsx`), 헤딩 hover 시 `#` 딥링크, 테이블은 flat(헤더 밑줄 + 행 구분선), 코드는 `slate-900` + hljs 팔레트. 링크/마커는 브랜드 그린.
