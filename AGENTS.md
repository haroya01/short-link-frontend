...
> **블로그 홈의 글 목록도 예외 없이 같은 컬럼이다.** 최신/인기/추천/팔로잉 탭과 검색 결과 모두 `FeedList`/`FeedCard` 목록 행, 탭 헤더도 `max-w-2xl`. 예전 메이슨리 카드 그리드는 코드 스크린샷 커버가 제목보다 먼저 보이고 흰 카드와 어두운 카드가 섞여 어수선해서 철거했다(2026-09). 카드 그리드가 남는 곳은 시리즈 탭(시리즈 덱 카드, 헤더도 `max-w-4xl xl:max-w-5xl`)뿐이다.
>
