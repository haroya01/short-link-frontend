...
`text-[17px]` 은 위 카드 스케일에 없지만 **블로그 읽기면(§10)의 목록 행 제목** 한 곳에만 쓰는 스텝이다 — 본문 18px 와 메타 14px 사이에서 제목이 한 단 더 또렷하게 앉는 크기(글쓰기 목록 · 시리즈 읽기 셸 · 다음 글 · 저장 카드). 이 용도(읽기면 행 제목) 밖으로 번지지 말 것.
...
> **예외 — 발견(browse) 면은 카드 그리드, 단 md+ 에서만.** 위 컬럼 불변식(§10.1)과 "카드 그리드 ❌"(§10.2)는 **읽기(reading) 면**(포스트 · 작가 · 태그 페이지)에만 적용된다. **블로그 홈의 둘러보기 면**(최신/인기/팔로잉/시리즈 탭 + 검색 + 인기 탭의 주제 칩 필터)은 의도적으로 **메이슨리 카드 그리드**(`DiscoveryCard`/`DiscoveryGrid`, `max-w-4xl xl:max-w-5xl` 3열)를 쓴다 — browse 는 훑어보는 면이라 그리드가 맞고, read 는 한 줄씩 읽는 면이라 컬럼이 맞다는 판단. 이 면에선 **탭 헤더도 그리드와 같은 폭(`max-w-4xl xl:max-w-5xl`)** 으로 둬, 탭 밑줄/구분선이 카드 그리드 폭과 연결되게 한다(헤더만 `max-w-2xl`이면 그리드보다 좁아 띠가 끊겨 보임). 근거·토큰은 `docs/discovery-feed.md`. 이 예외 밖(작가/태그/포스트)에서는 §10.1·§10.2 가 그대로 불변식이다.
>
> **모바일(<md)은 발견면도 1열 `FeedCard` 행이다.** 2열 타일은 한국어 제목이 ~8자에서 잘리고, CSS columns 메이슨리는 append/이미지 로드마다 전 카드를 재배치해 스크롤 중 콘텐츠가 손가락 밑에서 점프한다(리스트 행은 끝에만 붙어 점프가 없다). 메이슨리는 3열이 사는 md+ 전용 — `feed-infinite.tsx`/`following-feed.tsx` 의 `md:hidden` / `hidden md:block` 분기가 구현. 새 browse 면을 만들 때도 이 분기를 따른다.
>
> **#태그의 목적지는 하나** — `#태그` 텍스트/칩은 어디서든(카드·포스트 하단·레일·검색 빈화면) `/tags/[tag]` 읽기 페이지로 간다. in-place `?tag=` 필터는 **active 상태를 가진 칩 스트립**(인기 탭 `TrendingTopics`)에만 허용 — 같은 affordance 가 화면마다 다른 곳으로 가면 멘탈 모델이 깨진다.
...
### 10.7 모션 규칙
- **이징은 한 곡선**: `var(--ease)` (`cubic-bezier(0.16, 1, 0.3, 1)`, globals.css `:root`). Tailwind arbitrary 에선 `ease-[var(--ease)]` / `animate-[name_240ms_var(--ease)_both]`, 인라인 스타일에선 `"transform 320ms var(--ease)"`. framer-motion 처럼 var() 를 못 받는 곳만 `[0.16, 1, 0.3, 1]` 배열로 미러링하고 주석으로 출처를 남긴다. 새 cubic-bezier 하드코딩 ❌ (예외: §0 잠긴 warp 키프레임, 테마 wipe 의 의도된 soft 곡선). 마이크로 색/그림자 hover 는 §1 의 `ease-out 200ms` 를 그대로 써도 된다 — `var(--ease)` 가 담당하는 건 움직임(enter/exit/슬라이드/스케일) 전부다.
- **duration 3단**: 마이크로(hover/press/포커스) 150–220ms · 이동(시트/드롭다운/탭 전환) 240–320ms · 연출(페이지 전환/히어로 entrance) 440ms+. 새 값이 필요하면 이 3단 중 가까운 곳에 스냅.
- **모든 모션에 reduced-motion 처리** — CSS 는 `motion-reduce:` 또는 globals 의 글로벌 가드, JS 타이머/자동재생은 `matchMedia("(prefers-reduced-motion: reduce)")` (use-auto-slide / blog-bottom-nav 패턴).
...
### 10.9 활자 = Pretendard 단일 보이스 (디스플레이 세리프 금지)
- 전 표면 **Pretendard 단일**. 헤드라인은 weight(700/800) + `tracking-headline` 으로 editorial 밀도를 내고, 디스플레이 세리프 스왑은 하지 않는다.
- 세리프는 **두 차례 도입→철회**된 전력: ① Instrument Serif — 서양 세리프 이탤릭이 한글 우선 화면에서 겉돎. ② 마루 부리(MaruBuri) — 콘텐츠 제목·본문 헤딩 전반에 깔았다가 "명조 디스플레이가 올드해 보인다"로 당일 철회(2026-06, #701). 제3의 세리프를 제안하기 전에 이 이력부터 볼 것.
- 살아남은 인쇄물 어휘(세리프와 무관): 구분선(hr) = 애스터리즘 `✳ ✳ ✳`, 본문 링크 밑줄 = `decoration-1` + `underline-offset-[0.2em]`, `::selection` 다크 변형. 에디터(`.tiptap`)는 발행본과 동일하게.

---

## 11. 무대(Stage) 표면 모션 언어

> **적용 대상 = "한 번 보는 화면"만**: 랜딩 스크롤 여정 · 통계 데모 투어 · 방문자 게이트 · (예정) 웹 웰컴 · 발행 축하. 일하는 화면(폼·대시보드·피드·에디터·통계)은 §10.7 그대로 — 거기서 모션은 **정보일 때만**(예: 라이브 피드의 클릭 도착 맥박) 허용된다. 전략 근거는 vault `projects/kurl-web-stage-design.md`.

무대 표면에서 **완화되는 것**: duration 상한(연출 tier 를 넘는 스크롤 연동 시퀀스 허용), "눈에 띄지 말 것" 원칙(무대 모션은 보라고 만든 내러티브다).

무대 표면에서도 **완화되지 않는 것**:
- 이징 단일 곡선 `var(--ease)` (§10.7), 색은 브랜드 그린 스케일만, Pretendard 단일 보이스(§10.9)
- `prefers-reduced-motion` → **정적 완성 상태로 완결**(정보 손실 0). 과정 그 자체인 장식(스윕 밴드 등)은 숨김
- 성능: 스크롤 연동은 **CSS scroll-driven animations 만**(`animation-timeline` + `@supports` 게이트, 라이브러리 ❌), 영상·캔버스 ❌, LCP 회귀 ❌
- 미지원 브라우저 = `@supports` 밖의 기본 상태가 **완성 상태**여야 함(빈 화면 ❌)

### 11.1 어휘 4종 (globals.css `stage-*` — 새 연출은 이 어휘의 변주로 먼저 시도)
- **실-그리기** `.stage-thread` — 그린 실이 여정을 따라 그려짐 (`view()` 타임라인, celebrate-thread 의 일반화)
- **클릭-맥박** `.stage-pulse-ring` — 점 도착 + 파동 = 클릭 유입
- **형광-스윕** `.stage-sweep-host` — 강조 면 위로 형광 밴드 (`scroll(root)` 연동)
- **알약** `.stage-pill` — 단축 결과물의 물질화 (celebrate-pill 의 일반화)

### 11.2 플래그 계약 (lib/stage-flag.ts)
- 무대가 **기본 랜딩**(2026-07-23 졸업, SSR 첫 페인트부터 on). `?stage=off`(쿠키·`NEXT_PUBLIC_STAGE_DEFAULT=off` 비상 강등)로 레거시 구성이 안전핀으로 남아 있다 — 완전 철거 전까지 레거시 컴포넌트 삭제 금지.
- 우선순위: `?stage=on|off`(쿠키 고정) → `kurl_stage` 쿠키 → `NEXT_PUBLIC_STAGE_SPLIT` % 배정(A/B, PostHog `stage_variant_assigned`) → `NEXT_PUBLIC_STAGE_DEFAULT` → off.
- middleware 에서 Set-Cookie ❌ (랜딩 엣지 캐시 파괴 — NEXT_LOCALE 을 blog 에서 뺀 것과 같은 이유). 배정은 클라이언트에서.

---

## 12. 유리(Glass) 크롬 재질

> **유리 = 크롬에만**: 헤더 · 하단 탭바 · 오프라인 배너 · 쿠키 배너 · 플로팅 캡슐 · 오버레이 새시. **콘텐츠 카드에 유리 ❌** — §1/§10 flat 종이 철학 위반이자 가독성 사고(NN/g). iOS 앱의 확정 문법 "종이 본문, 액체 크롬"의 웹 쌍둥이다.

- 재질은 **`.glass-chrome`(바) / `.glass-capsule`(알약) / `.glass-panel`(떠 있는 패널: 다이얼로그·시트·드롭다운·검색) / `.glass-toast`(다크 고정 토스트)** 네 클래스만 소유한다(globals.css) — 유사-유리 직접 조합 ❌. `.glass-panel`/`.glass-toast` 는 **립+그림자를 토큰이 소유**하므로 적용처에 shadow-* 유틸 금지. 하단 부착 크롬(탭바·쿠키)은 `.glass-lip`(윗변 빛) 동반.
- 보더/그림자는 컴포넌트 몫(면마다 border-t/b 방향이 달라서). 유리 위 보더는 `slate-200/60`(다크 `slate-800/60`) 헤어라인.
- 가드레일: blur 10–14px 고정 · **backdrop-filter 애니메이션 ❌** · 큰 콘텐츠 면적 ❌ · 텍스트 대비 4.5:1(배경 불투명도 0.55 미만 금지, 바 계열 0.62+) · `@supports` 불투명 폴백 + `prefers-reduced-transparency` 완전 불투명.
- 다크는 `.dark .glass-*` 가 자동 처리 — 컴포넌트에 `dark:bg-*` 중복 금지.
- **버튼 리퀴드(2026-07-24)**: ui/button.tsx variant 가 내장 — 틴트 유지 + /90(채움)·/60(outline) 반투명 + backdrop-blur-md + 상단 립(inset 은 ring 합성 보존 위해 `shadow-[inset_…]` 유틸로만). hover = 응고(불투명 복귀). 채움 CTA 불투명도 0.9 미만 ❌(대비 가드). iOS glassCapsule 버튼 문법의 웹 파리티.
- **상단 크롬 = 상시 유리 캡슐**: 헤더는 "투명 sticky 외곽(h-14) + 상시 캡슐 판(inset-x-3·max-w-[1248px]·rounded-full·상하 1.5 인셋·부양 그림자)" 단일 상태 — 첫 화면부터 떠 있는 유리 카드로 보인다(스크롤 조건부 축소는 "스샷과 실물이 다르다" 지적으로 철회, 2026-07-23). 콘텐츠 행은 캡슐 위 relative. 상태 전환이 없으므로 애니메이션 규칙 무관, 히스테리시스 훅도 제거됨. nav.tsx·app-header.tsx 가 기준 구현.
...
- 2026-06: 모바일 발견면 1열 `FeedCard` 행(§10.1 — 메이슨리는 md+ 전용), 모션 규칙 §10.7 신설(`--ease` 단일 곡선·duration 3단·`shadow-card-hover` 토큰·`BlogChromeLink` 크롬 소프트 내비)
